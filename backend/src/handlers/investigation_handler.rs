...
pub async fn entity_explanation(
    ...
    let risk = get_json(&state.http_client, &url).await?;
    Ok(ApiResponse::success(json!({
        "entity_id": id,
        "identifier": identifier,
        "risk_score": risk.get("risk_score").cloned().unwrap_or(Value::Null),
        "risk_level": risk.get("risk_level").cloned().unwrap_or(Value::Null),
        "explanation": risk.get("factors").cloned().unwrap_or(json!([])),
        "top_reasons": risk.get("top_reasons").cloned().unwrap_or(json!([])),
    })))
}
